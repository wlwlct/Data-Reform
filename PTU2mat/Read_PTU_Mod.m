%%%%This is program mainly from picoquant demo. modified slightly for
%%%%make the ASCII code in right column and rows. No marker used during the
%%%%measurement. Combine with 'import function' (generated by matlab
%%%%itself), use to generate *.mat file for easier application in the
%%%%future.


function [MeasDesc_Resolution]=Read_PTU_Mod(filename, pathname) % Read PicoQuant Unified TTTR Files
...
% used for further analysis
    ...
    % write Header
    if (isT2)
      fprintf(fpout, '\nrecord# Type Ch TimeTag TrueTime/ps\n');
    else
...
function GotPhoton(TimeTag, Channel, DTime)
  ...
  cnt_ph = cnt_ph + 1;
  if(isT2)
      fprintf(fpout,'\n%i,9,%1x,%f,%8.0lf', RecNum, Channel, TimeTag, (TimeTag * MeasDesc_GlobalResolution * 1e12));
  else
      fprintf(fpout,'\n%i,9,%1x,%i,%.6f,%i', RecNum, Channel, TimeTag, (TimeTag * MeasDesc_GlobalResolution * 1e9+DTime*MeasDesc_Resolution*1*10^(9)), DTime);
  end;
end
...
function GotOverflow(Count,Channel)
  ...
  cnt_ov = cnt_ov + Count;
  if(isT2)
  fprintf(fpout,'\n%i,5,%u,0,0', RecNum, Count);    
  else
  %fprintf(fpout,'\n%i,5,%u,0,0,0', RecNum, Count);
  end
...
function ReadHT3(Version)
    ...
    for i = 1:TTResult_NumberOfRecords
        ...
        if special == 0   % this means a regular input channel
           ...
           GotPhoton(TimeTag, channel, dtime);
            ...
            if channel == 63  % overflow of nsync occured
              if (nsync == 0) || (Version == 1) % if nsync is zero it is an old style single oferflow or old Version
                ...
                GotOverflow(1,channel);
              else         % otherwise nsync indicates the number of overflows - THIS IS NEW IN FORMAT V2.0
                OverflowCorrection = OverflowCorrection + T3WRAPAROUND * nsync;%%%%%%%%%%%%%%%%%%%%%%%%This is the useful part
                GotOverflow(nsync,channel);
              end;    
            end;
